...
def main(argv=None) -> int:
    ...
    if not pdfs:
        print(f"No hay PDF en {PDF_DIR}.\n"
              "Esta comprobación usa los que descargó el rescate de texto; "
              "ejecútala en el ordenador donde estén.")
        return 1

    print("=" * 74)
    print("¿TEXTO ÍNTEGRO O RESUMEN? · web frente a PDF del número")
    print("=" * 74)
    print(f"{len(pdfs)} PDF disponibles en disco\n")

    filas = []
    with psycopg.connect(cfg.database_url, row_factory=dict_row) as conn:
        for ruta in pdfs[:args.limite]:
            # data/pdfs/<publicacion>-<numero>.pdf
